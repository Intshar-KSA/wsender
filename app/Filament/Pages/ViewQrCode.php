    ...
    public function getShareableLink(): string
{
    ...
    private function loadQrCode(string $qrCodeText)
    {
        ...
        try {
            ...
            if (isset($response['status']) && $response['status'] === 'done' && isset($response['qrCode'])) {
                ...
                } elseif (isset($response['detail']) && $response['detail'] === 'You are already authorized') {
            ...
            if (strpos($e->getMessage(), 'Profile not found') !== false) {
                // إذا كان البروفايل غير موجود، أنشئ بروفايل جديد
                $this->handleProfileNotFound();
            } elseif (strpos($e->getMessage(), 'You are already authorized') !== false) {
                $this->updateDeviceStatus('active');
            } else {
    ...
    private function updateDeviceStatus($status)
    {
        ...
        if ($device) {
            ...
            session()->flash('success', _('Connection successful.'));
            Notification::make()
            ...
            Notification::make()
                ->title(_('This device is not available'))
                ->danger()
                ->send();
